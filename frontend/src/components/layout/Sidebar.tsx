...
function getFolderIcon(name: string, large = false): React.ReactNode {
  ...
  if (lower === 'sent' || lower === 'sent mail' || lower === '[gmail]/sent mail') {
    return (
      <svg className={cls} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
    ...
    >
      ...
      <div className={isCompact ? 'p-1.5' : 'p-3'}>
        <Button
          ...
          className={isCompact ? 'w-10 h-10 !p-0 mx-auto flex items-center justify-center' : 'w-full'}
      ...
      <nav
        ref={folderListRef}
        className={isCompact ? 'flex-1 overflow-y-auto px-1.5' : 'flex-1 overflow-y-auto px-2'}
        aria-label="Mail folders"
      >
        <ul className={isCompact ? 'space-y-1' : 'space-y-0.5'} role="list">
          {folders.map((folder) => {
      ...
      <div className={[
        ...
        isCompact ? 'flex flex-col items-center gap-2 py-3 px-1.5' : 'p-3',
      ].join(' ')}>
        ...
        {isCompact ? (
          <div
            className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-[var(--color-accent)]/10 text-[var(--color-accent)] text-sm font-semibold"
            title={email ?? ''}
